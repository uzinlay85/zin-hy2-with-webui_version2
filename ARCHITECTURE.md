...
- **Auth**: Uses external HTTP authentication, pointing to the FastAPI Web Panel (`http://127.0.0.1:3000/auth`).
...
- **Authentication Endpoint (`/auth`)**: Hysteria 2 calls this endpoint on every client connection attempt. The panel checks the SQLite database for valid credentials, expiration status, data limits, and active device limits (by cross-referencing Hysteria's `/online` API).
...
- **`install.sh`**: Handles the complete one-click setup. 
  ...
  - Modifies `sysctl.conf` for BBR and network optimization.
  - Sets up UFW firewall and port hopping (`iptables` REDIRECT).
  - Fetches Hysteria 2 binary and configures Systemd services.
